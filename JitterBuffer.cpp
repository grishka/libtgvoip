...
JitterBuffer::JitterBuffer(MediaStreamItf *out,
                           uint32_t step): bufferPool(JITTER_SLOT_SIZE,
	                                   JITTER_SLOT_COUNT)
{
	if(out) {
	...
	this->step = step;
	memset(slots, 0,
	       sizeof(jitter_packet_t)*JITTER_SLOT_COUNT);
	minDelay = 6;
	lostCount = 0;
	needBuffering = true;
	tickCount = 0;
	...
	lostPackets = 0;
	if(step < 30) {
		minMinDelay = (uint32_t)
		              ServerConfig::GetSharedInstance()->GetInt("jitter_min_delay_20",
		                      6);
		maxMinDelay = (uint32_t)
		              ServerConfig::GetSharedInstance()->GetInt("jitter_max_delay_20",
		                      25);
		maxUsedSlots = (uint32_t)
		               ServerConfig::GetSharedInstance()->GetInt("jitter_max_slots_20",
		                       50);
	} else if(step < 50) {
		minMinDelay = (uint32_t)
		              ServerConfig::GetSharedInstance()->GetInt("jitter_min_delay_40",
		                      4);
		maxMinDelay = (uint32_t)
		              ServerConfig::GetSharedInstance()->GetInt("jitter_max_delay_40",
		                      15);
		maxUsedSlots = (uint32_t)
		               ServerConfig::GetSharedInstance()->GetInt("jitter_max_slots_40",
		                       30);
	} else {
		minMinDelay = (uint32_t)
		              ServerConfig::GetSharedInstance()->GetInt("jitter_min_delay_60",
		                      1);
		maxMinDelay = (uint32_t)
		              ServerConfig::GetSharedInstance()->GetInt("jitter_max_delay_60",
		                      10);
		maxUsedSlots = (uint32_t)
		               ServerConfig::GetSharedInstance()->GetInt("jitter_max_slots_60",
		                       20);
	}
	lossesToReset = (uint32_t)
	                ServerConfig::GetSharedInstance()->GetInt("jitter_losses_to_reset",
	                        20);
	resyncThreshold =
	    ServerConfig::GetSharedInstance()->GetDouble("jitter_resync_threshold",
	            1.0);
...
void JitterBuffer::SetMinPacketCount(
    uint32_t count)
{
	if(minDelay == count) {
		return;
...
size_t JitterBuffer::CallbackIn(unsigned char
                                *data, size_t len, void *param)
{
	//((JitterBuffer*)param)->HandleInput(data, len);
	return 0;
}

size_t JitterBuffer::CallbackOut(unsigned char
                                 *data, size_t len, void *param)
{
	return ((JitterBuffer *)param)->HandleOutput(data,
	        len, 0);
}

void JitterBuffer::HandleInput(unsigned char
                               *data, size_t len, uint32_t timestamp)
{
	jitter_packet_t pkt;
	pkt.size = len;
	pkt.buffer = data;
...
void JitterBuffer::Reset()
{
	...
	gotSinceReset = 0;
	expectNextAtTime = 0;
	memset(deviationHistory, 0,
	       sizeof(deviationHistory));
	deviationPtr = 0;
}


size_t JitterBuffer::HandleOutput(unsigned char
                                  *buffer, size_t len, int offsetInSteps)
{
	jitter_packet_t pkt;
	pkt.buffer = buffer;
	pkt.size = len;
...
int JitterBuffer::GetInternal(jitter_packet_t *
                              pkt, int offset)
{
	...
	int64_t timestampToGet = nextTimestamp + offset *
	                         (int32_t)step;

	int i;
	for(i = 0; i < JITTER_SLOT_COUNT; i++) {
		if(slots[i].buffer != NULL &&
		        slots[i].timestamp == timestampToGet) {
			break;
		}
	}
	...
	if(i < JITTER_SLOT_COUNT) {
		if(pkt && pkt->size < slots[i].size) {
			LOGE("jitter: packet won't fit into provided buffer of %d (need %d)",
			     int(slots[i].size), int(pkt->size));
		} else {
			if(pkt) {
				pkt->size = slots[i].size;
				pkt->timestamp = slots[i].timestamp;
				memcpy(pkt->buffer, slots[i].buffer,
				       slots[i].size);
			}
		}
		bufferPool.Reuse(slots[i].buffer);
	...
	LOGW("jitter: found no packet for timestamp %lld (last put = %d, lost = %d)",
	     (long long int)timestampToGet, lastPutTimestamp,
	     lostCount);
	...
	if(!needBuffering) {
		...
		if(lostCount >= lossesToReset ||
		        (gotSinceReset > minDelay * 25 &&
		         lostSinceReset > gotSinceReset / 2)) {
			LOGW("jitter: lost %d packets in a row, resetting",
			     lostCount);
			//minDelay++;
			dontIncMinDelay = 16;
			dontDecMinDelay += 128;
...
void JitterBuffer::PutInternal(jitter_packet_t *
                               pkt)
{
	...
	if(wasReset) {
		wasReset = false;
		nextTimestamp = ((int64_t)pkt->timestamp)
		                - step * minDelay;
		LOGI("jitter: resyncing, next timestamp = %lld (step=%d, minDelay=%d)",
		     (long long int)nextTimestamp, step, minDelay);
	}
	...
	if(pkt->timestamp < nextTimestamp) {
		LOGW("jitter: would drop packet with timestamp %d because it is late but not hopelessly",
		     pkt->timestamp);
		latePacketCount++;
		lostPackets--;
	} else if(pkt->timestamp < nextTimestamp - 1) {
		LOGW("jitter: dropping packet with timestamp %d because it is too late",
		     pkt->timestamp);
		latePacketCount++;
		return;
	}
...
void JitterBuffer::Tick()
{
	...
	memmove(&lateHistory[1], lateHistory,
	        63 * sizeof(int));
	...
	memmove(&delayHistory[1], delayHistory,
	        63 * sizeof(int));
	...
	uint32_t stddevDelay = (uint32_t)ceil(
	                           stddev * 2 * 1000 / step);
	...
	if(stddevDelay != minDelay) {
		...
		if((diff > 0 && dontIncMinDelay == 0) || (diff < 0 &&
		        dontDecMinDelay == 0)) {
	...
	if(!adjustingDelay) {
		if (((minDelay == 1 ? (avgDelay >= 3) :
		        (avgDelay >= minDelay / 2)) &&
		        delayHistory[0] > minDelay && avgLate16 <= 0.1 &&
		        absolutelyNoLatePackets && dontDecMinDelay < 32 &&
		        min > minDelay) /*|| (avgRecvTimeDiff>0 && delayHistory[0]>minDelay/2 && avgRecvTimeDiff*1000<step/2)*/) {
...
void JitterBuffer::GetAverageLateCount(
    double *out)
{
	double avgLate64 = 0, avgLate32 = 0, avgLate16 = 0;
	int i;
