...
public:
	ChannelBuffer(size_t num_frames,
		...
		for (size_t i = 0; i < num_allocated_channels_;
		        ++i) {
			for (size_t j = 0; j < num_bands_; ++j) {
				channels_[j * num_allocated_channels_ + i] =
				    &data_[i * num_frames_ + j *
				           num_frames_per_band_];
				bands_[i * num_bands_ + j] = channels_[j *
				                                       num_allocated_channels_ + i];
			}
		}
	}
	...
	T **Slice(T **slice, size_t start_frame) {
		const ChannelBuffer<T> *t = this;
		return const_cast<T **>(t->Slice(slice,
		                                 start_frame));
	}
	...
	void set_num_channels(size_t num_channels) {
		RTC_DCHECK_LE(num_channels,
		              num_allocated_channels_);
		num_channels_ = num_channels;
	}

	void SetDataForTesting(const T *data,
	                       size_t size) {
...
public:
	IFChannelBuffer(size_t num_frames,
	                size_t num_channels, size_t num_bands = 1);
	~IFChannelBuffer();
	...
	size_t num_channels() const {
		return ivalid_ ? ibuf_.num_channels() :
		       fbuf_.num_channels();
	}
	void set_num_channels(size_t num_channels) {
