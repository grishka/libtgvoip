...
typedef struct NoiseSuppressionFixedC_ {
	...
	int16_t                 noiseEstDensity[SIMULT *
	                                        HALF_ANAL_BLOCKL];
	...
	uint32_t
	whiteNoiseLevel;  // Initial noise estimate.
	// Initial magnitude spectrum estimate.
	uint32_t
	initMagnEst[HALF_ANAL_BLOCKL];
	// Pink noise parameters:
	int32_t
	pinkNoiseNumerator;  // Numerator.
	int32_t
	pinkNoiseExp;  // Power of freq.
	int
	minNorm;  // Smallest normalization factor.
	int
	zeroInputSignal;  // Zero input signal flag.
	...
	int
	blockIndex;  // Frame index counter.
 ...
 *                        -1 - Error
 */
int32_t WebRtcNsx_InitCore(NoiseSuppressionFixedC *
                           inst, uint32_t fs);
...
int WebRtcNsx_set_policy_core(
    NoiseSuppressionFixedC *inst, int mode);
 ...
 */
void WebRtcNsx_ProcessCore(NoiseSuppressionFixedC *
                           inst,
...
typedef void (*NormalizeRealBuffer)(
    NoiseSuppressionFixedC *inst,
    const int16_t *in,
    int16_t *out);
extern NormalizeRealBuffer
WebRtcNsx_NormalizeRealBuffer;

// Compute speech/noise probability.
// Intended to be private.
void WebRtcNsx_SpeechNoiseProb(
    NoiseSuppressionFixedC *inst,
...
void WebRtcNsx_NoiseEstimationNeon(
    NoiseSuppressionFixedC *inst,
    uint16_t *magn,
    uint32_t *noise,
    int16_t *q_noise);
void WebRtcNsx_SynthesisUpdateNeon(
    NoiseSuppressionFixedC *inst,
    int16_t *out_frame,
    int16_t gain_factor);
void WebRtcNsx_AnalysisUpdateNeon(
    ...
    int16_t *out,
    int16_t *new_speech);
void WebRtcNsx_PrepareSpectrumNeon(
    NoiseSuppressionFixedC *inst,
    int16_t *freq_buff);
#endif
...
void WebRtcNsx_SynthesisUpdate_mips(
    NoiseSuppressionFixedC *inst,
    int16_t *out_frame,
    int16_t gain_factor);
void WebRtcNsx_AnalysisUpdate_mips(
    ...
    int16_t *out,
    int16_t *new_speech);
void WebRtcNsx_PrepareSpectrum_mips(
    NoiseSuppressionFixedC *inst,
    int16_t *freq_buff);
void WebRtcNsx_NormalizeRealBuffer_mips(
    NoiseSuppressionFixedC *inst,
    const int16_t *in,
    int16_t *out);
#if defined(MIPS_DSP_R1_LE)
void WebRtcNsx_Denormalize_mips(
    NoiseSuppressionFixedC *inst,
    int16_t *in,
    int factor);
#endif
