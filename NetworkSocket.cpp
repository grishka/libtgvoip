...
{
	ipv6Timeout =
	    ServerConfig::GetSharedInstance()->GetDouble("nat64_fallback_timeout",
	            3);
	failed = false;
}
...
std::string NetworkSocket::GetLocalInterfaceInfo(
...
{
	return (uint16_t) ((rand() % (MAX_UDP_PORT
	                              - MIN_UDP_PORT)) + MIN_UDP_PORT);
}
...
{
	IPv4Address *self4 = dynamic_cast<IPv4Address *>
	                     (this);
	IPv4Address *other4 = dynamic_cast<IPv4Address *>((
	                          NetworkAddress *)&other);
	if(self4 && other4) {
		return self4->GetAddress() == other4->GetAddress();
	}
	IPv6Address *self6 = dynamic_cast<IPv6Address *>
	                     (this);
	IPv6Address *other6 = dynamic_cast<IPv6Address *>((
	                          NetworkAddress *)&other);
	if(self6 && other6) {
		return memcmp(self6->GetAddress(),
		              other6->GetAddress(), 16) == 0;
	}
	return false;
}
...
#ifndef _WIN32
	this->address =
	    NetworkSocketPosix::StringToV4Address(addr);
#else
	this->address =
	    NetworkSocketWinsock::StringToV4Address(addr);
#endif
}
...
{
#ifndef _WIN32
	return NetworkSocketPosix::V4AddressToString(
	           address);
#else
	return NetworkSocketWinsock::V4AddressToString(
	           address);
#endif
}
...
{
#ifndef _WIN32
	NetworkSocketPosix::StringToV6Address(addr,
	                                      this->address);
#else
	NetworkSocketWinsock::StringToV6Address(addr,
	                                        this->address);
#endif
}
