...
AudioInputPulse::AudioInputPulse(std::string
                                 devID)
{
	isRecording = false;
	isConnected = false;
	...
	pa_context_set_state_callback(context,
	                              AudioInputPulse::ContextStateCallback, this);
	...
	stream = pa_stream_new(context, "libtgvoip capture",
	                       &sample_specifications, NULL);
	if(!stream) {
	...
	pa_stream_set_state_callback(stream,
	                             AudioInputPulse::StreamStateCallback, this);
	pa_stream_set_read_callback(stream,
	                            AudioInputPulse::StreamReadCallback, this);
...
void AudioInputPulse::Configure(uint32_t
...
void AudioInputPulse::Start()
{
	...
	pa_operation_unref(pa_stream_cork(stream, 0,
	                                  AudioInputPulse::StreamSuccessCallback,
	                                  mainloop));
}
...
void AudioInputPulse::SetCurrentDevice(
    std::string devID)
{
	currentDevice = devID;
	...
	int err = pa_stream_connect_record(stream,
	                                   devID == "default" ? NULL : devID.c_str(),
	                                   &bufferAttr, (pa_stream_flags_t)streamFlags);
...
void AudioInputPulse::EnumerateDevices(
    std::vector<AudioInputDevice> &devs)
{

}

void AudioInputPulse::ContextStateCallback(
...
void AudioInputPulse::StreamStateCallback(
...
void AudioInputPulse::StreamReadCallback(
    pa_stream *stream, size_t requestedBytes,
    void *userdata)
{
	((AudioInputPulse *)userdata)->StreamReadCallback(
	    stream, requestedBytes);
}
    ...
    pa_stream *stream, size_t requestedBytes)
{
	...
	while (bytesRemaining > 0) {
		...
		int err = pa_stream_peek(stream,
		                         (const void **) &buffer, &bytesToFill);
		CHECK_ERROR(err, "pa_stream_peek");

		if(isRecording) {
			if(remainingDataSize + bytesToFill > sizeof(
			            remainingData)) {
				LOGE("Capture buffer is too big (%d)",
				     (int)bytesToFill);
			}
			memcpy(remainingData + remainingDataSize, buffer,
			       bytesToFill);
...
void AudioInputPulse::StreamSuccessCallback(
