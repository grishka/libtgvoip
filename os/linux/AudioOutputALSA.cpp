...
AudioOutputALSA::AudioOutputALSA(std::string
                                 devID)
{
	isPlaying = false;
	handle = NULL;
	...
	LOAD_FUNCTION(lib, "snd_pcm_close",
	              _snd_pcm_close);
	LOAD_FUNCTION(lib, "snd_pcm_writei",
	              _snd_pcm_writei);
...
void AudioOutputALSA::Configure(uint32_t
...
void AudioOutputALSA::Start()
{
	...
	isPlaying = true;
	start_thread(thread, AudioOutputALSA::StartThread,
	             this);
}
...
void AudioOutputALSA::RunThread()
{
	...
	while(isPlaying) {
		...
		frames = _snd_pcm_writei(handle, buffer,
		                         BUFFER_SIZE);
		if (frames < 0) {
...
void AudioOutputALSA::SetCurrentDevice(
    std::string devID)
{
...
void AudioOutputALSA::EnumerateDevices(
