...
AudioInputWave::AudioInputWave(std::string
                               deviceID)
{
	isRecording = false;
...
void AudioInputWave::Configure(uint32_t
...
void AudioInputWave::Start()
{
	...
	MMRESULT res;
	for(int i = 0; i < 4; i++) {
		res = waveInPrepareHeader(hWaveIn, &buffers[i],
		                          sizeof(WAVEHDR));
		CHECK_ERROR(res, "waveInPrepareHeader failed");
		res = waveInAddBuffer(hWaveIn, &buffers[i],
		                      sizeof(WAVEHDR));
...
void AudioInputWave::Stop()
{
	...
	for(int i = 0; i < 4; i++) {
		res = waveInUnprepareHeader(hWaveIn, &buffers[i],
		                            sizeof(WAVEHDR));
		CHECK_ERROR(res, "waveInUnprepareHeader failed");
	}
}

void AudioInputWave::WaveInProc(HWAVEIN hwi,
                                UINT uMsg, DWORD_PTR dwInstance,
                                DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
	if(uMsg == WIM_DATA) {
		((AudioInputWave *)dwInstance)->OnData((
		        WAVEHDR *)dwParam1);
	}
}
...
void AudioInputWave::OnData(WAVEHDR *hdr)
{
	...
	MMRESULT res = waveInAddBuffer(hWaveIn, hdr,
	                               sizeof(WAVEHDR));
	CHECK_ERROR(res, "waveInAddBuffer failed");
}

void AudioInputWave::EnumerateDevices(
...
void AudioInputWave::SetCurrentDevice(
    std::string deviceID)
{
	...
	if(deviceID == "default") {
		MMRESULT res = waveInOpen(&hWaveIn, WAVE_MAPPER,
		                          &format, (DWORD_PTR)AudioInputWave::WaveInProc,
		...
		for(UINT i = 0; i < num; i++) {
			...
			if(name == deviceID) {
				MMRESULT res = waveInOpen(&hWaveIn, i, &format,
				                          (DWORD_PTR)AudioInputWave::WaveInProc,
