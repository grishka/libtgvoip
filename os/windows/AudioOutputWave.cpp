...
AudioOutputWave::AudioOutputWave(std::string
                                 deviceID)
{
	isPlaying = false;
	hWaveOut = NULL;
...
void AudioOutputWave::Configure(uint32_t
...
void AudioOutputWave::Start()
{
	...
	for(int i = 0; i < 4; i++) {
		MMRESULT res = waveOutPrepareHeader(hWaveOut,
		                                    &buffers[i], sizeof(WAVEHDR));
		CHECK_ERROR(res, "waveOutPrepareHeader failed");
		//InvokeCallback((unsigned char*)buffers[i].lpData, buffers[i].dwBufferLength);
		ZeroMemory(buffers[i].lpData,
		           buffers[i].dwBufferLength);
		res = waveOutWrite(hWaveOut, &buffers[i],
		                   sizeof(WAVEHDR));
...
void AudioOutputWave::Stop()
{
	...
	for(int i = 0; i < 4; i++) {
		res = waveOutUnprepareHeader(hWaveOut, &buffers[i],
		                             sizeof(WAVEHDR));
...
void AudioOutputWave::WaveOutProc(HWAVEOUT hwo,
                                  UINT uMsg, DWORD_PTR dwInstance,
                                  DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
	if(uMsg == WOM_DONE) {
		((AudioOutputWave *)dwInstance)->OnBufferDone((
		            WAVEHDR *)dwParam1);
	}
}
...
void AudioOutputWave::OnBufferDone(WAVEHDR *hdr)
{
	...
	InvokeCallback((unsigned char *)hdr->lpData,
	               hdr->dwBufferLength);
	hdr->dwFlags &= ~WHDR_DONE;
	MMRESULT res = waveOutWrite(hWaveOut, hdr,
	                            sizeof(WAVEHDR));
}

void AudioOutputWave::EnumerateDevices(
...
void AudioOutputWave::SetCurrentDevice(
    std::string deviceID)
{
	...
	if(deviceID == "default") {
		MMRESULT res = waveOutOpen(&hWaveOut, WAVE_MAPPER,
		                           &format, (DWORD_PTR)AudioOutputWave::WaveOutProc,
		...
		for(UINT i = 0; i < num; i++) {
			...
			if(name == deviceID) {
				MMRESULT res = waveOutOpen(&hWaveOut, i, &format,
				                           (DWORD_PTR)AudioOutputWave::WaveOutProc,
				                           (DWORD_PTR)this, CALLBACK_FUNCTION | WAVE_MAPPED);
				CHECK_ERROR(res, "waveOutOpen failed");
	...
	if(isPlaying) {
		MMRESULT res;
		for(int i = 0; i < 4; i++) {
			res = waveOutPrepareHeader(hWaveOut, &buffers[i],
			                           sizeof(WAVEHDR));
			CHECK_ERROR(res, "waveOutPrepareHeader failed");
			res = waveOutWrite(hWaveOut, &buffers[i],
			                   sizeof(WAVEHDR));
