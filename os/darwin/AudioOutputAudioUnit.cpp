...
void AudioOutputAudioUnit::Configure(
    ...
    uint32_t channels)
{
	io->Configure(sampleRate, bitsPerSample,
	              channels);
}
...
void AudioOutputAudioUnit::EnableLoudspeaker(
    bool enabled)
{
    ...
    AudioBufferList *ioData)
{
	...
	for(i = 0; i < ioData->mNumberBuffers; i++) {
		...
		unsigned int samples = buf.mDataByteSize / sizeof(
		                           int16_t);
		for (k = 0; k < samples; k++) {
			int16_t absolute = (int16_t)abs(*((int16_t *)
			                                  buf.mData + k));
			if (absolute > absVal) {
				absVal = absolute;
			}
