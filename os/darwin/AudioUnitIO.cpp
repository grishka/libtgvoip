...
void AudioUnitIO::ProcessAudioSessionAcquired()
{
	...
	inputComponent = AudioComponentFindNext(NULL,
	                                        &desc);
	status = AudioComponentInstanceNew(inputComponent,
	                                   &unit);

	if(configured) {
		ActuallyConfigure(cfgSampleRate, cfgBitsPerSample,
		                  cfgChannels);
	}
}
...
void AudioUnitIO::Configure(uint32_t sampleRate,
                            uint32_t bitsPerSample, uint32_t channels)
{
	...
	runFakeIO = true;
	start_thread(fakeIOThread,
	             AudioUnitIO::StartFakeIOThread, this);
	set_thread_priority(fakeIOThread,
	                    get_thread_max_priority());

	if(haveAudioSession) {
		ActuallyConfigure(sampleRate, bitsPerSample,
		                  channels);
	} else {
...
void AudioUnitIO::ActuallyConfigure(
    ...
    uint32_t channels)
{
	...
	status = AudioUnitSetProperty(unit,
	                              ...
	                              kAudioUnitScope_Global, kInputBus, &flag,
	                              sizeof(flag));
	...
	status = AudioUnitSetProperty(unit,
	                              ...
	                              kAudioUnitScope_Input, kOutputBus, &audioFormat,
	                              sizeof(audioFormat));
	CHECK_AU_ERROR(status,
	               "Error setting output format");
	status = AudioUnitSetProperty(unit,
	                              kAudioUnitProperty_StreamFormat,
	                              kAudioUnitScope_Output, kInputBus, &audioFormat,
	                              sizeof(audioFormat));
	CHECK_AU_ERROR(status,
	               "Error setting input format");

	AURenderCallbackStruct callbackStruct;

	callbackStruct.inputProc =
	    AudioUnitIO::BufferCallback;
	...
	CHECK_AU_ERROR(status,
	               "Error setting output buffer callback");
	status = AudioUnitSetProperty(unit,
	...
	CHECK_AU_ERROR(status,
	               "Error initializing AudioUnit");
	status = AudioOutputUnitStart(unit);
	CHECK_AU_ERROR(status,
	               "Error starting AudioUnit");
}

OSStatus AudioUnitIO::BufferCallback(
    void *inRefCon,
...
{
	((AudioUnitIO *)inRefCon)->BufferCallback(
	    ioActionFlags, inTimeStamp, inBusNumber,
	    inNumberFrames, ioData);
	return noErr;
}

void AudioUnitIO::BufferCallback(
    AudioUnitRenderActionFlags *ioActionFlags,
    const AudioTimeStamp *inTimeStamp, UInt32 bus,
    UInt32 numFrames, AudioBufferList *ioData)
{
	...
	if(bus == kOutputBus) {
		...
		if(input && inputEnabled) {
			inBufferList.mBuffers[0].mDataByteSize = 10240;
			AudioUnitRender(unit, ioActionFlags, inTimeStamp,
			                bus, numFrames, &inBufferList);
			input->HandleBufferCallback(&inBufferList);
		}
	}
}

void AudioUnitIO::AttachInput(AudioInputAudioUnit
                              *i)
{
	assert(input == NULL);
	input = i;
...
void AudioUnitIO::AttachOutput(
    AudioOutputAudioUnit *o)
{
	assert(output == NULL);
	output = o;
}
