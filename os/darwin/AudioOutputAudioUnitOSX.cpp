    ...
    std::string deviceID)
{
	...
	AudioComponent component = AudioComponentFindNext(
	                               NULL, &inputDesc);
	status = AudioComponentInstanceNew(component,
	                                   &unit);
	CHECK_AU_ERROR(status,
	               "Error creating AudioUnit");
	...
	status = AudioUnitSetProperty(unit,
	                              ...
	                              sizeof(flag));
	CHECK_AU_ERROR(status,
	               "Error enabling AudioUnit output");
	flag = 0;
	status = AudioUnitSetProperty(unit,
	                              kAudioOutputUnitProperty_EnableIO,
	                              kAudioUnitScope_Input, kInputBus, &flag,
	                              sizeof(flag));
	CHECK_AU_ERROR(status,
	               "Error enabling AudioUnit input");

	char model[128];
	memset(model, 0, sizeof(model));
	size_t msize = sizeof(model);
	int mres = sysctlbyname("hw.model", model, &msize,
	                        NULL, 0);
	if(mres == 0) {
		LOGV("Mac model: %s", model);
		isMacBookPro = (strncmp("MacBookPro", model,
		                        10) == 0);
	}
	...
	status = AudioObjectSetPropertyData(
	             kAudioObjectSystemObject, &propertyAddress, 0,
	...
	AudioObjectAddPropertyListener(
	    ...
	    AudioOutputAudioUnit::DefaultDeviceChangedCallback,
	    this);
	...
	callbackStruct.inputProc =
	    AudioOutputAudioUnit::BufferCallback;
	...
	CHECK_AU_ERROR(status,
	               "Error setting input buffer callback");
...
AudioOutputAudioUnit::~AudioOutputAudioUnit()
{
	...
	if(isMacBookPro && sysDevID &&
	        AudioObjectHasProperty(sysDevID,
	                               &dataSourceProp)) {
		AudioObjectRemovePropertyListener(sysDevID,
		                                  &dataSourceProp,
		                                  AudioOutputAudioUnit::DefaultDeviceChangedCallback,
		                                  this);
	}
...
void AudioOutputAudioUnit::Configure(
...
void AudioOutputAudioUnit::Stop()
{
	...
	OSStatus status = AudioOutputUnitStart(unit);
	CHECK_AU_ERROR(status,
	               "Error stopping AudioUnit");
}

OSStatus AudioOutputAudioUnit::BufferCallback(
    void *inRefCon,
...
{
	AudioOutputAudioUnit *input =
	    (AudioOutputAudioUnit *) inRefCon;
...
void AudioOutputAudioUnit::HandleBufferCallback(
    AudioBufferList *ioData)
{
	int i;
	unsigned int k;
	...
	for(i = 0; i < ioData->mNumberBuffers; i++) {
		...
		memcpy(buf.mData, remainingData,
		       buf.mDataByteSize);
		remainingDataSize -= buf.mDataByteSize;
		memmove(remainingData,
		        remainingData + buf.mDataByteSize,
		        remainingDataSize);
	}
}


void AudioOutputAudioUnit::EnumerateDevices(
    std::vector<AudioOutputDevice> &devs)
{
	...
	if(kAudioHardwareNoError != status) {
		LOGE("AudioObjectGetPropertyDataSize (kAudioHardwarePropertyDevices) failed: %i",
		     status);
		return;
	}

	UInt32 deviceCount = (UInt32)(dataSize / sizeof(
	                                  AudioDeviceID));


	AudioDeviceID *audioDevices = (AudioDeviceID *)(
	                                  malloc(dataSize));
	...
	if(kAudioHardwareNoError != status) {
		LOGE("AudioObjectGetPropertyData (kAudioHardwarePropertyDevices) failed: %i",
		     status);
		free(audioDevices);
		audioDevices = NULL;
		return;
	...
	for(UInt32 i = 0; i < deviceCount; ++i) {
		...
		status = AudioObjectGetPropertyData(
		             audioDevices[i], &propertyAddress, 0, NULL,
		             &dataSize, &deviceUID);
		if(kAudioHardwareNoError != status) {
			LOGE("AudioObjectGetPropertyData (kAudioDevicePropertyDeviceUID) failed: %i",
			     status);
			continue;
		}
		...
		status = AudioObjectGetPropertyData(
		             ...
		             &dataSize, &deviceName);
		if(kAudioHardwareNoError != status) {
			LOGE("AudioObjectGetPropertyData (kAudioDevicePropertyDeviceNameCFString) failed: %i",
			     status);
			continue;
		}
		...
		status = AudioObjectGetPropertyDataSize(
		             ...
		             &dataSize);
		if(kAudioHardwareNoError != status) {
			LOGE("AudioObjectGetPropertyDataSize (kAudioDevicePropertyStreamConfiguration) failed: %i",
			     status);
			continue;
		}

		AudioBufferList *bufferList = (AudioBufferList *)(
		                                  malloc(dataSize));
		...
		status = AudioObjectGetPropertyData(
		             ...
		             &dataSize, bufferList);
		if(kAudioHardwareNoError != status ||
		        0 == bufferList->mNumberBuffers) {
			if(kAudioHardwareNoError != status) {
				LOGE("AudioObjectGetPropertyData (kAudioDevicePropertyStreamConfiguration) failed: %i",
				     status);
			}
			free(bufferList);
			bufferList = NULL;
		...
		char buf[1024];
		CFStringGetCString(deviceName, buf, 1024,
		                   kCFStringEncodingUTF8);
		dev.displayName = std::string(buf);
		CFStringGetCString(deviceUID, buf, 1024,
		                   kCFStringEncodingUTF8);
...
void AudioOutputAudioUnit::SetCurrentDevice(
    std::string deviceID)
{
	...
	LOGD("Switched playback device, new sample rate %d",
	     hardwareSampleRate);
	...
	if(status == noErr) {
		...
		LOGD("CoreAudio buffer size for output device is %u frames (%u ms)",
		     bufferFrameSize, estimatedDelay);
	}

	if(isMacBookPro) {
		if(AudioObjectHasProperty(outputDevice,
		                          &dataSourceProp)) {
			UInt32 dataSource;
			size = 4;
			AudioObjectGetPropertyData(outputDevice,
			                           &dataSourceProp, 0, NULL, &size, &dataSource);
			SetPanRight(dataSource == 'ispk');
			AudioObjectAddPropertyListener(outputDevice,
			                               &dataSourceProp,
			                               AudioOutputAudioUnit::DefaultDeviceChangedCallback,
			                               this);
		} else {
...
void AudioOutputAudioUnit::SetPanRight(
    bool panRight)
{
	LOGI("%sabling pan right on macbook pro",
	     panRight ? "En" : "Dis");
	int32_t channelMap[] = {panRight ? -1 : 0, 0};
	OSStatus status = AudioUnitSetProperty(unit,
	                                       kAudioOutputUnitProperty_ChannelMap,
	                                       kAudioUnitScope_Global, kOutputBus, channelMap,
	                                       sizeof(channelMap));
	CHECK_AU_ERROR(status,
	               "Error setting channel map");
}

OSStatus AudioOutputAudioUnit::DefaultDeviceChangedCallback(
    AudioObjectID inObjectID,
    UInt32 inNumberAddresses,
    const AudioObjectPropertyAddress *inAddresses,
    void *inClientData)
{
	AudioOutputAudioUnit *self = (AudioOutputAudioUnit *)
	                             inClientData;
	if(inAddresses[0].mSelector ==
	        kAudioHardwarePropertyDefaultOutputDevice) {
		LOGV("System default input device changed");
		if(self->currentDevice == "default") {
			self->SetCurrentDevice(self->currentDevice);
		}
	} else if(inAddresses[0].mSelector ==
	          kAudioDevicePropertyDataSource) {
		UInt32 dataSource;
		UInt32 size = 4;
		AudioObjectGetPropertyData(inObjectID,
		                           inAddresses, 0, NULL, &size, &dataSource);
